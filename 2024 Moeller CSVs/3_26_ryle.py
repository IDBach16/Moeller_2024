...
Original file is located at
    https://colab.research.google.com/drive/1TL5RlafjUl7h2qqOMPkt-3X6-9hy28G1
"""
...
import pandas as pd
df = pd.read_csv('drive/My Drive/3.26 Ryle.csv')

df.head()

filtered_df = df[(df['Balls'] == 1) & (df['Strikes'] == 1) & (df['BatterTeam'] == 'Moeller')]
...
plt.xlabel('Outcome')
plt.ylabel('%')
plt.title('1-0 Count Outcome % 3.26 Ryle')
plt.show()
...
for pitcher in df3['Pitcher'].unique():
  ...
  plt.title('3.26 Ryle Strike Percentage by Pitch Type - ' + pitcher)
  plt.show()
...
plt.xlabel('Outcome')
plt.ylabel('%')
plt.title('Breaking Ball/Changeup Outcome % 3.26 Ryle')
plt.show()

df4 = df.loc[df['is_moeller_pitcher']==1, ['Pitcher', 'Attack Zone', 'is_strike']].groupby(['Pitcher', 'Attack Zone'], as_index=False).mean()
df4 = df4.rename(columns={'is_strike':'Strike%'})
df5 = df.loc[df['is_moeller_pitcher']==1, ['Pitcher', 'Attack Zone', 'is_strike']].groupby(['Pitcher', 'Attack Zone'], as_index=False).count()
df5 = df5.rename(columns={'is_strike':'Count'})
df_attack = df4.merge(df5, on=['Pitcher', 'Attack Zone'])
df_attack

for pitcher in df_attack['Pitcher'].unique():
  print(pitcher)
  df_plt = df_attack.loc[df_attack['Pitcher']==pitcher]
  sns.barplot(data=df_plt, x='Strike%', y='Attack Zone')

  ax = plt.gca()
  ax.xaxis.set_major_formatter(ticker.PercentFormatter(xmax=1))

  plt.title('3.26 Ryle Strike Percentage by Attack Zone - ' + pitcher)
  plt.show()
